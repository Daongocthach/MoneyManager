...
function CategoriesIncome() {
    ...
    const categories = useSelector(state => state.categoriesIncome.categoriesIncome)
    return (
        <View style={{ flex: 1, backgroundColor: colorScheme == 'dark' ? 'black' : 'white' }}>
            <TouchableOpacity onPress={() => navigation.navigate('AddCategoryIncome', { id: categories.length })}
                style={{
            ...
            <ScrollView>
                {Array.isArray(categories) && categories.map((category, index) => (
                    <TouchableOpacity style={{ ...styles.flexRow, margin: 15 }} key={index}
                        onPress={() => navigation.navigate('AddCategoryIncome', {
                            category: category
                        })}>
